...
func TestHealthyFilter(t *testing.T){
   ...
   result, err = f.Filter(&dockerclient.ContainerConfig{}, nodesAllHealth)
   assert.NoError(t, err)
   assert.Equal(t, result,nodesAllHealth)

   result, err = f.Filter(&dockerclient.ContainerConfig{}, nodesPartHealth)
   assert.NoError(t, err)
   assert.Len(t, result,1)
   assert.Equal(t, result[0], nodesPartHealth[1])

   result, err = f.Filter(&dockerclient.ContainerConfig{}, nodesNoHealth)
